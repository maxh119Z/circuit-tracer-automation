...
For each (slug, min_group_size) combination, invoke validate_anthropic_groups.py
...
Usage:
    ...
    python run_validation_sweep.py --min-sizes 2,3,4 --standardize-names
...
PIPELINE_SCRIPT = Path(__file__).resolve().parent / "validate_anthropic_groups.py"
...
def discover_slugs(require_manual: bool = True) -> list[str]:
    """Return artifact slugs that have both auto groups and (optionally) a manual file."""
...
def run_one(
    ...
    """Invoke validate_anthropic_groups.py as a subprocess; return the parsed report dict."""
    artifact_dir = ARTIFACTS_ROOT / slug
    report_path = artifact_dir / f"validation_report_{DESCRIPTION_VARIANT}_{GROUPING_VARIANT}_anthropic.json"

    env = os.environ.copy()
    env["CURRENT_SLUG"] = slug
    env["VALIDATION_MIN_GROUP_SIZE"] = str(min_group_size)
    env["VALIDATION_STANDARDIZE_NAMES"] = "1" if standardize_names else "0"
    ...
    log.info("→ %s  min_group_size=%d  std=%s", slug, min_group_size, standardize_names)
    proc = subprocess.run(
...
def report_to_rows(slug: str, min_group_size: int, report: dict) -> list[dict]:
    """Flatten one validation report into one row per condition."""
    rows: list[dict] = []
    ...
    for cond in ALL_CONDITIONS:
        ...
        if c.get("skipped"):
            rows.append({
                ...
                "standardize_names": report.get("standardize_names", False),
                "condition": cond,
                "skipped": True,
        ...
        rows.append({
            ...
            "d1_mean": d1.get("mean_accuracy"),
            "d2_mean": d2.get("mean_accuracy"),
        })
    return rows
...
def main() -> None:
    ...
    parser.add_argument("--standardize-names", action="store_true",
                        help="Run validate_groups with VALIDATION_STANDARDIZE_NAMES=1.")
    ...
    if args.slugs:
        ...
        if not slugs:
            log.error(
                "No slugs found. Run fetch_neuronpedia_groups.py to populate manual_groups.json, "
    ...
    log.info("Standardize names: %s", args.standardize_names)
    log.info("Output: %s", out_path)

    fieldnames = [
        "slug", "prompt", "min_group_size", "difficulty", "standardize_names",
    ...
    with open(out_path, "a", encoding="utf-8", newline="") as f:
        ...
        for slug in slugs:
            for size in min_sizes:
                report = run_one(slug, size, args.standardize_names, conditions)
                if report is None:
