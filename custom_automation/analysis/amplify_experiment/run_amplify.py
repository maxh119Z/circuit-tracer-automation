...
def find_answer_features(graph: dict, correct_answer: str,
    ...
    Matches both plain concept groups (e.g. 'Europe') and say-X groups (e.g. 'say Europe').
    Falls back to clerp matching if no groups available.
    """
    ...
    if feature_groups is not None:
        ...
        if matching_groups:
            print(f"    Answer groups matched: {matching_groups}")
            results = []
            ...
            return results
        print(f"    WARNING: no group matched '{correct_answer}' — falling back to clerp")

    # Clerp fallback
    pattern = re.compile(r"\b" + re.escape(answer_lower) + r"\b")
    results = []
